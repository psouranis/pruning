...
from finetune_eval import evaluate, finetune
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="Iterations to run pruning along with finetuning",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    model = pruner.iterative_prune(
        model=model,
        skip_modules_by_name=IGNORE,
        skip_modules_by_type=(nn.LayerNorm),
        iterations=args.iterations,
        dataloader=finetune_dataloader,
        eval_dataloader=eval_dataloader,
        finetune=finetune,
        evaluate=evaluate,
        args_finetune=args_finetune,
    )
